...
template <class T>
struct is_empty
    : aux::integral_constant<bool, sizeof(is_empty_base<T, none_type>) == sizeof(none_type)>
{
};
template <class>
...
template <int N>
struct make_index_sequence_impl : concat<typename make_index_sequence_impl<N / 2>::type,
...
template <class... Ts, class... T1s, class... T2s, class... T3s, class... T4s, class... T5s,
          class... T6s, class... T7s, class... T8s, class... T9s, class... T10s, class... T11s,
          class... T12s, class... T13s, class... T14s, class... T15s, class... T16s, class... Us>
struct join<type_list<Ts...>, type_list<T1s...>, type_list<T2s...>, type_list<T3s...>,
            type_list<T4s...>, type_list<T5s...>, type_list<T6s...>, type_list<T7s...>,
            type_list<T8s...>, type_list<T9s...>, type_list<T10s...>, type_list<T11s...>,
            type_list<T12s...>, type_list<T13s...>, type_list<T14s...>, type_list<T15s...>,
            type_list<T16s...>, Us...>
    : join<type_list<Ts..., T1s..., T2s..., T3s..., T4s..., T5s..., T6s..., T7s..., T8s..., T9s...,
                     T10s..., T11s..., T12s..., T13s..., T14s..., T15s..., T16s...>,
           Us...>
{
};
...
struct is_unique<type<Rs...>, T, Ts...>
    : conditional_t<is_base_of<type<T>, inherit<type<Rs>...>>::value, false_type,
                    is_unique<type<Rs..., T>, Ts...>>
{
};
template <class... Ts>
...
template <class T>
struct pool_type_impl<T&, aux::enable_if_t<aux::is_constructible<T>::value &&
...
struct missing_ctor_parameter
{
    ...
    template <class U, __BOOST_SML_REQUIRES(!aux::is_base_of<pool_type_base, U>::value &&
    ...
    constexpr operator TMissing&() const
    {
        static_assert(missing_ctor_parameter<TMissing>::value,
                      "State Machine is missing a constructor parameter! Check out the "
                      "`missing_ctor_parameter` error to see the "
                      "missing type.");
    }
#endif
};
...
struct pool : pool_type<Ts>...
{
    ...
    constexpr pool(init, const pool<TArgs...>& p)
        : pool_type<Ts>(
              try_get<aux::remove_const_t<aux::remove_reference_t<aux::remove_pointer_t<Ts>>>>(
                  &p))...
    {
    }
    template <class... TArgs>
...
struct zero_wrapper : TExpr
{
    ...
    constexpr const TExpr& get() const
    {
        return *this;
    }
};
...
{
#if defined(_MSC_VER) && !defined(__clang__)
    return detail::get_type_name<T, 65>(__FUNCSIG__,
                                        make_index_sequence<sizeof(__FUNCSIG__) - 65 - 8>{});
#elif defined(__clang__) && (__clang_major__ >= 12)
    return detail::get_type_name<T, 50>(
        __PRETTY_FUNCTION__, make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 50 - 2>{});
#elif defined(__clang__)
    return detail::get_type_name<T, 63>(
        __PRETTY_FUNCTION__, make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 63 - 2>{});
#elif defined(__GNUC__)
    return detail::get_type_name<T, 69>(
        __PRETTY_FUNCTION__, make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 69 - 2>{});
#endif
}
...
struct string<T>
{
    ...
    constexpr static auto c_str_impl(...)
    {
...
struct sm_policy;
template <class TEvent>
using get_event = aux::conditional_t<aux::is_base_of<internal_event, TEvent>::value,
...
{
    using type =
        aux::join_t<get_event<TEvent>, typename sm<TSrc>::events, typename sm<TDst>::events>;
};
template <class, class TEvent>
struct get_sub_internal_events_impl
{
    using type = aux::conditional_t<aux::is_base_of<internal_event, TEvent>::value,
...
using get_all_events =
    aux::join_t<typename get_all_events_impl<typename Ts::src_state, typename Ts::dst_state,
                                             typename Ts::event>::type...>;
template <class... Ts>
using get_sub_internal_events = aux::join_t<
    typename get_sub_internal_events_impl<typename Ts::src_state, typename Ts::event>::type...,
...
using get_initial_states =
    aux::join_t<typename aux::conditional<Ts::initial, aux::type_list<typename Ts::src_state>,
                                          aux::type_list<>>::type...>;
template <class... Ts>
using get_history_states = aux::join_t<
    typename aux::conditional<!Ts::history && Ts::initial, aux::type_list<typename Ts::src_state>,
                              aux::type_list<>>::type...>;
template <class>
...
using get_non_empty_t =
    aux::join_t<typename aux::conditional<aux::is_empty<Ts>::value, aux::type_list<>,
                                          aux::type_list<Ts>>::type...>;
template <class... Ts>
...
struct sub_sm<sm_impl<sm_policy<T>>>
{
    ...
    constexpr static sm_impl<sm_policy<T, aux::identity<U>, TPolicies...>>&
    get(aux::pool_type<sm_impl<sm_policy<T, aux::identity<U>, TPolicies...>>>* object)
    {
        return static_cast<aux::pool_type<sm_impl<sm_policy<T, aux::identity<U>, TPolicies...>>>&>(
                   *object)
            .value;
    }
    template <class... TPolicies>
    constexpr static sm_impl<sm_policy<T, TPolicies...>>&
    ...
    template <class... TPolicies>
    constexpr static const sm_impl<sm_policy<T, TPolicies...>>&
    cget(const aux::pool_type<sm_impl<sm_policy<T, TPolicies...>>>* object)
    {
        return static_cast<const aux::pool_type<sm_impl<sm_policy<T, TPolicies...>>>&>(*object)
            .value;
    }
};
template <class T, class U>
struct sub_sm<sm_impl<sm_policy<T, aux::identity<U>>>>
{
    ...
    cget(const aux::pool_type<sm_impl<sm_policy<T, aux::identity<U>, TPolicies...>>>* object)
    {
        return static_cast<
                   const aux::pool_type<sm_impl<sm_policy<T, aux::identity<U>, TPolicies...>>>&>(
                   *object)
            .value;
    }
};
...
struct transitions<T, Ts...>
{
    ...
    constexpr static bool execute(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs,
                                  typename SM::state_t& current_state)
    {
        if (aux::get<T>(sm.transitions_)
                .execute(event, sm, deps, subs, current_state, typename SM::has_entry_exits{}))
        {
            return true;
        }
...
struct transitions_sub<sm<TSM>, T, Ts...>
{
    ...
    constexpr static bool execute(const anonymous& event, SM& sm, TDeps& deps, TSubs& subs,
                                  typename SM::state_t& current_state)
    {
    ...
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr static bool execute_impl(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs,
                                       typename SM::state_t& current_state)
    {
        const auto handled = sub_sm<sm_impl<TSM>>::get(&subs).process_event(event, deps, subs);
        return handled ? handled
    ...
    constexpr static bool execute_impl(const back::on_entry<_, TEvent>& event, SM& sm, TDeps& deps,
    ...
    template <class _, class TEvent, class SM, class TDeps, class TSubs>
    constexpr static bool execute_impl(const back::on_exit<_, TEvent>& event, SM& sm, TDeps& deps,
                                       TSubs& subs, typename SM::state_t& current_state)
    {
        sub_sm<sm_impl<TSM>>::get(&subs).process_event(event, deps, subs);
        transitions<T, Ts...>::execute(event, sm, deps, subs, current_state);
...
{
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr static bool execute(const TEvent& event, SM&, TDeps& deps, TSubs& subs,
                                  typename SM::state_t&)
    {
...
struct get_mapping<event<E>, event<E>, T, R>
    : aux::type_list<event_mappings<
          E, aux::apply_t<unique_mappings_t, aux::join_t<typename R::types, typename T::types>>>>
{
};
...
struct extend_mapping
    : aux::join_t<
          typename get_mapping<typename T::element_type, typename Ts::element_type, T, Ts>::type...>
{
};
template <class T, class... Ts>
...
{
    using type =
        unique_mappings_impl<aux::type<aux::inherit<T1>, extend_mapping_t<T2, Rs...>>, Ts...>;
};
template <class T1, class T2, class... Rs, class... Ts>
struct conditional_mapping<false, aux::type<T1, aux::inherit<Rs...>>, T2, Ts...>
{
    using type = unique_mappings_impl<
        aux::type<aux::inherit<T1, aux::type<typename T2::element_type>>, aux::inherit<T2, Rs...>>,
        Ts...>;
};
template <class T1, class T2, class... Rs, class... Ts>
...
struct mappings<aux::pool<Ts...>>
    : unique_mappings_t<event_mappings<
          typename Ts::event,
          aux::inherit<state_mappings<typename Ts::src_state, aux::type_list<Ts>>>>...>
{
};
template <class T>
...
struct get_state_mapping
{
    using type = aux::conditional_t<
        aux::is_same<decltype(get_state_mapping_impl<T, TUnexpected>((TMappings*)0)),
...
unique_mappings_t<T1Mappings..., T2Mappings...>
get_event_mapping_impl(event_mappings<T, aux::inherit<T1Mappings...>>*,
...
struct get_event_mapping_impl_helper
    : aux::conditional<aux::is_same<transitions<aux::true_type>,
                                    decltype(get_event_mapping_impl<_>((TMappings*)0))>::value,
...
struct jump_table
{
    template <int, class TMappings, class sm_impl, class State, class TEvent, class TDeps,
              class TSubs>
    constexpr static bool dispatch(sm_impl&, State&, const TEvent&, TDeps&, TSubs&,
                                   const aux::type_list<>&)
    {
        return false;
    }
    template <int, class TMappings, class sm_impl, class State, class TEvent, class TDeps,
              class TSubs, class... TStates>
    static bool dispatch(sm_impl& self, State& current_state, const TEvent& event, TDeps& deps,
                         TSubs& subs, const aux::type_list<TStates...>&)
    {
        ...
        constexpr static dispatch_table_t
            dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(sizeof...(TStates))] = {
                &get_state_mapping_t<TStates, TMappings, typename sm_impl::has_unexpected_events>::
...
struct branch_stm
{
    ...
    template <int N, class TMappings, class sm_impl, class State, class TEvent, class TDeps,
              class TSubs, class TState, class... TStates>
    constexpr static bool dispatch(sm_impl& self, State& current_state, const TEvent& event,
                                   TDeps& deps, TSubs& subs,
                                   const aux::type_list<TState, TStates...>&)
    {
        return current_state == N ? get_state_mapping_t<TState, TMappings,
...
struct switch_stm
{
                                   ...
                                   const aux::type_list<TState, TStates...>&)
    {
        ...
        {
        case N:
            return get_state_mapping_t<TState, TMappings, typename sm_impl::has_unexpected_events>::
                template execute<TEvent, sm_impl, TDeps, TSubs>(event, self, deps, subs,
                                                                current_state);
        default:
            return dispatch<N + 1, TMappings>(self, current_state, event, deps, subs,
                                              aux::type_list<TStates...>{});
        }
    }
};
...
struct fold_expr
{
    template <class TMappings, int... Ns, class sm_impl, class State, class TEvent, class TDeps,
              class TSubs, class... TStates>
    constexpr static bool dispatch_impl(sm_impl& self, State& current_state,
                                        aux::index_sequence<Ns...>, const TEvent& event,
                                        TDeps& deps, TSubs& subs, const aux::type_list<TStates...>&)
    {
    ...
    {
        return dispatch_impl<TMappings>(self, current_state,
                                        aux::make_index_sequence<sizeof...(TStates)>{}, event, deps,
                                        subs, states);
    }
};
#endif
...
constexpr void log_state_change(const aux::type<no_policy>&, TDeps&, const TSrcState&,
                                const TDstState&)
{
}
template <class SM, class TLogger, class TDeps, class TSrcState, class TDstState>
constexpr void log_state_change(const aux::type<TLogger>&, TDeps& deps, const TSrcState&,
                                const TDstState&)
{
    return static_cast<aux::pool_type<TLogger&>&>(deps).value.template log_state_change<SM>(
        get_state_name_t<TSrcState>{}, get_state_name_t<TDstState>{});
}
...
constexpr void log_action(const aux::type<TLogger>&, TDeps& deps, const TAction& action,
                          const TEvent& event)
{
    return static_cast<aux::pool_type<TLogger&>&>(deps).value.template log_action<SM>(action,
                                                                                      event);
}
template <class SM, class TLogger, class TDeps, class TAction, class TEvent>
...
{
    return static_cast<aux::pool_type<TLogger&>&>(deps).value.template log_action<SM>(action.get(),
                                                                                      event);
}
...
constexpr void log_guard(const aux::type<TLogger>&, TDeps& deps, const TGuard& guard,
                         const TEvent& event, bool result)
{
    return static_cast<aux::pool_type<TLogger&>&>(deps).value.template log_guard<SM>(guard, event,
                                                                                     result);
}
template <class SM, class TLogger, class TDeps, class TGuard, class TEvent>
constexpr void log_guard(const aux::type<TLogger>&, TDeps& deps,
                         const aux::zero_wrapper<TGuard>& guard, const TEvent& event, bool result)
{
    return static_cast<aux::pool_type<TLogger&>&>(deps).value.template log_guard<SM>(guard.get(),
                                                                                     event, result);
}
} // namespace policies
} // namespace back
...
struct thread_safe : aux::pair<thread_safety_policy__, thread_safe<TLock>>
{
    ...
    constexpr auto create_lock()
    {
        struct lock_guard
        {
            constexpr explicit lock_guard(TLock& lock) : lock_{lock}
            {
                lock_.lock();
            }
            ~lock_guard()
...
struct sm_policy
{
    ...
    using sm = SM;
    using thread_safety_policy = decltype(get_policy<no_policy, policies::thread_safety_policy__>(
        (aux::inherit<TPolicies...>*)0));
    using defer_queue_policy = decltype(get_policy<no_policy, policies::defer_queue_policy__>(
        (aux::inherit<TPolicies...>*)0));
    using process_queue_policy = decltype(get_policy<no_policy, policies::process_queue_policy__>(
        (aux::inherit<TPolicies...>*)0));
    using logger_policy =
        decltype(get_policy<no_policy, policies::logger_policy__>((aux::inherit<TPolicies...>*)0));
    using testing_policy =
        decltype(get_policy<no_policy, policies::testing_policy__>((aux::inherit<TPolicies...>*)0));
    using dont_instantiate_statemachine_class_policy =
        decltype(get_policy<no_policy, policies::dont_instantiate_statemachine_class_policy__>(
            (aux::inherit<TPolicies...>*)0));
...
template <class, class T>
struct callable
    : decltype(test_callable<aux::inherit<aux::conditional_t<__is_class(T), T, aux::none_type>,
                                          callable_fallback>>(0))
{
};
                                    ...
                                    aux::none_type, typename TSM::sm>
{
    ...
    using initial_states_t =
        aux::apply_t<aux::unique_t, aux::apply_t<get_initial_states, transitions_t>>;
    ...
    using has_history_states = aux::integral_constant<bool, aux::size<initial_states_t>::value !=
                                                                aux::size<history_states_t>::value>;
    using sub_internal_events_t =
        aux::apply_t<aux::unique_t, aux::apply_t<get_sub_internal_events, transitions_t>>;
    using events_t =
        aux::apply_t<aux::unique_t, aux::join_t<sub_internal_events_t,
    ...
    using state_t =
        aux::conditional_t<(aux::size<states_t>::value > 0xFF), unsigned short, aux::byte>;
    ...
    constexpr decltype(auto) try_get_without_instantiating(const TPool& p) const
    {
        static_assert(
            ...
            "When policy sml::dont_instantiate_statemachine_class is used, you have to provide a "
            "reference to an "
            "instance of the transition table type (boost::sml::sm< your_transition_table_type >) "
            "as well as a reference to instances of all sub-statemachine types as constructor "
    ...
    constexpr bool process_event(const TEvent& event, TDeps& deps, TSubs& subs)
    {
        ...
        do
        {
            ...
            } while (process_defer_events(deps, subs, changed, aux::type<defer_queue_t<TEvent>>{},
                                          events_t{}));
...
#else
        (void)aux::swallow{
            0, (current_state_[region++] = aux::get_id<state_t, TStates>((states_ids_t*)0), 0)...};
#endif
    }
...
#if BOOST_SML_DISABLE_EXCEPTIONS
        return process_event_impl<get_event_mapping_t<get_generic_t<TEvent>, mappings>>(
            event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
#else
        return process_event_except_impl<get_event_mapping_t<get_generic_t<TEvent>, mappings>>(
    ...
    constexpr bool process_internal_events(const TEvent& event, TDeps& deps, TSubs& subs)
    {
        policies::log_process_event<sm_t>(aux::type<logger_t>{}, deps, event);
#if BOOST_SML_DISABLE_EXCEPTIONS
        return process_event_impl<get_event_mapping_t<get_mapped_t<TEvent>, mappings>>(
            event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
#else
        return process_event_except_impl<get_event_mapping_t<get_mapped_t<TEvent>, mappings>>(
            event, deps, subs, has_exceptions{});
#endif
    }
    template <class TEvent, class TDeps, class TSubs, class... Ts,
    ...
    template <class TEvent, class TDeps, class TSubs,
              __BOOST_SML_REQUIRES(aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value)>
    constexpr bool process_internal_generic_event(const TEvent& event, TDeps& deps, TSubs& subs,
    ...
    template <class TEvent, class TDeps, class TSubs,
              __BOOST_SML_REQUIRES(aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value &&
                                   !aux::is_base_of<get_mapped_t<TEvent>, events_ids_t>::value)>
    constexpr bool process_internal_event(const TEvent& event, TDeps& deps, TSubs& subs,
                                          state_t& current_state)
    {
    ...
    {
        return dispatch_t::template dispatch<0, TMappings>(*this, current_state_[0], event, deps,
                                                           subs, states);
    }
                                      ...
                                      aux::index_sequence<Ns...>)
    {
        auto handled = false;
#if defined(__cpp_fold_expressions)
        ((handled |= dispatch_t::template dispatch<0, TMappings>(*this, current_state_[Ns], event,
                                                                 deps, subs, states)),
         ...);
#else
        (void)aux::swallow{0, (handled |= dispatch_t::template dispatch<0, TMappings>(
    ...
    constexpr bool process_event_impl(const TEvent& event, TDeps& deps, TSubs& subs,
                                      ...
                                      state_t& current_state)
    {
        return dispatch_t::template dispatch<0, TMappings>(*this, current_state, event, deps, subs,
                                                           states);
    }
#if !BOOST_SML_DISABLE_EXCEPTIONS
    template <class TMappings, class TEvent, class TDeps, class TSubs>
    constexpr bool process_event_except_impl(const TEvent& event, TDeps& deps, TSubs& subs,
                                             aux::false_type)
    {
    ...
    template <class TMappings, class TEvent, class TDeps, class TSubs>
    constexpr bool process_event_except_impl(const TEvent& event, TDeps& deps, TSubs& subs,
                                             state_t& current_state, aux::false_type)
    {
    ...
    constexpr bool process_defer_events(TDeps&, TSubs&, const bool, const aux::type<no_policy>&,
    ...
    bool process_defer_events(TDeps& deps, TSubs& subs, const bool handled,
                              const aux::type<TDeferQueue>&, const aux::type_list<TEvents...>&)
    {
        bool processed_events = false;
        if (handled)
        {
            using dispatch_table_t = bool (sm_impl::*)(TDeps&, TSubs&, const void*);
            const static dispatch_table_t dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(
                sizeof...(TEvents))] = {&sm_impl::process_event_no_defer<TDeps, TSubs, TEvents>...};
    ...
    template <class TDeps, class TSubs, class... TEvents>
    constexpr bool process_queued_events(TDeps&, TSubs&, bool&, const aux::type<no_policy>&,
                                         const aux::type_list<TEvents...>&)
    {
        return false;
    }
    ...
    constexpr bool process_event_no_queue(TDeps& deps, TSubs& subs, const void* data)
    {
        ...
        return process_event_impl<get_event_mapping_t<TEvent, mappings>>(
            event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
#else
        return process_event_except_impl<get_event_mapping_t<TEvent, mappings>>(event, deps, subs,
                                                                                has_exceptions{});
#endif
    }
    template <class TDeps, class TSubs, class TDeferQueue, class... TEvents>
                               ...
                               const aux::type<TDeferQueue>&, const aux::type_list<TEvents...>&)
    {
        using dispatch_table_t = bool (sm_impl::*)(TDeps&, TSubs&, const void*);
        const static dispatch_table_t dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(
            sizeof...(TEvents))] = {&sm_impl::process_event_no_queue<TDeps, TSubs, TEvents>...};
    ...
    constexpr void visit_current_states(const TVisitor& visitor, const aux::type_list<TStates...>&,
...
class sm
{
    using sm_t = typename TSM::sm;
    using logger_t = typename TSM::logger_policy::type;
    using logger_dep_t = aux::conditional_t<aux::is_same<no_policy, logger_t>::value,
...
private:
    using sm_all_t = aux::apply_t<
        ...
        aux::join_t<
            aux::conditional_t<
                aux::is_same<no_policy,
                             typename TSM::dont_instantiate_statemachine_class_policy>::value,
                aux::type_list<sm_t>, aux::none_type>,
            aux::apply_t<get_sm_t, state_machines>>>;

    using sub_sms_t = aux::apply_t<
        aux::pool, typename convert_to_sm<
                       TSM, aux::apply_t<aux::unique_t, aux::apply_t<get_sub_sms, states>>>::type>;
    using deps = aux::apply_t<merge_deps, transitions_t>;
    using deps_t =
        aux::apply_t<aux::pool,
                     aux::apply_t<aux::unique_t, aux::join_t<deps, sm_all_t, logger_dep_t,
...
public:
    ...
    template <class TDeps,
              __BOOST_SML_REQUIRES(!aux::is_same<aux::remove_reference_t<TDeps>, sm>::value)>
    constexpr explicit sm(TDeps&& deps)
        : deps_{aux::init{}, aux::pool<TDeps>{deps}}, sub_sms_{aux::pool<TDeps>{deps}}
    {
        aux::get<sm_impl<TSM>>(sub_sms_).start(deps_, sub_sms_);
    }
    template <class... TDeps,
              __BOOST_SML_REQUIRES((sizeof...(TDeps) > 1) && aux::is_unique_t<TDeps...>::value)>
    constexpr explicit sm(TDeps&&... deps)
        : deps_{aux::init{}, aux::pool<TDeps...>{deps...}}, sub_sms_{aux::pool<TDeps...>{deps...}}
    {
        aux::get<sm_impl<TSM>>(sub_sms_).start(deps_, sub_sms_);
    ...
    constexpr bool process_event(const TEvent& event)
    {
        return aux::get<sm_impl<TSM>>(sub_sms_).process_event(unexpected_event<_, TEvent>{event},
                                                              deps_, sub_sms_);
    }
    ...
    constexpr void visit_current_states(const TVisitor& visitor) const
    {
        ...
        aux::cget<sm_impl_t>(sub_sms_).visit_current_states(visitor, states_t{},
                                                            aux::make_index_sequence<regions>{});
    }
    template <class T = aux::identity<sm_t>, class TState,
              __BOOST_SML_REQUIRES(
                  sm_impl<typename TSM::template rebind<typename T::type>>::regions == 1)>
    ...
    template <
        ...
        __BOOST_SML_REQUIRES(sizeof...(TStates) ==
    ...
    constexpr bool is(const TStates&...) const
    {
        ...
        visit_current_states<T>(
            ...
            {
                (void)state;
                result &= (aux::get_id<state_t, typename decltype(state)::type>((states_ids_t*)0) ==
                           state_ids[i++]);
            });
        return result;
    }
    template <class T = aux::identity<sm_t>, class... TStates,
              __BOOST_SML_REQUIRES(!aux::is_same<no_policy, typename TSM::testing_policy>::value &&
                                   aux::always<T>::value)>
...
template <class T, class E>
auto args__(int) -> aux::function_traits_t<
    decltype(&T::__BOOST_SML_TEMPLATE_KEYWORD operator()<back::get_event_t<E>>)>;
template <class T, class E>
...
template <class T, class TEvent, class TSM, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<const TEvent&>&,
                                 const back::exception<T, TEvent>& event, TSM&, TDeps&)
{
    return event.exception_;
}
template <class... TEvents, class TEvent, class TSM, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<back::defer<TEvents...>>&, const TEvent, TSM& sm,
                                 TDeps&)
{
    return back::defer<TEvents...>{sm.defer_};
}
template <class... TEvents, class TEvent, class TSM, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<back::process<TEvents...>>&, const TEvent, TSM& sm,
                                 TDeps&)
{
...
struct call<TEvent, aux::type_list<>, TLogger>
{
    ...
    constexpr static auto execute_impl(const aux::type<bool>&, T object, const TEvent& event,
                                       TDeps& deps)
    {
    ...
    template <class TSM, class T, class TDeps>
    constexpr static auto execute_impl(const aux::type<void>&, T object, const TEvent& event,
                                       TDeps& deps)
    {
...
private:
    ...
    constexpr void for_all(const aux::index_sequence<Ns...>&, const TEvent& event, TSM& sm,
                           TDeps& deps, TSubs& subs)
    {
#if defined(__cpp_fold_expressions)
        (call<TEvent, args_t<Ts, TEvent>, typename TSM::logger_t>::execute(aux::get_by_id<Ns>(&a),
                                                                           event, sm, deps, subs),
         ...);
#else
        (void)aux::swallow{0, (call<TEvent, args_t<Ts, TEvent>, typename TSM::logger_t>::execute(
...
private:
    template <int... Ns, class TEvent, class TSM, class TDeps, class TSubs>
    constexpr auto for_all(const aux::index_sequence<Ns...>&, const TEvent& event, TSM& sm,
                           TDeps& deps, TSubs& subs)
    {
#if defined(__cpp_fold_expressions)
        ...
        (void)aux::swallow{
            0,
            (result = result && call<TEvent, args_t<Ts, TEvent>, typename TSM::logger_t>::execute(
                                    aux::get_by_id<Ns>(&g), event, sm, deps, subs),
             0)...};
        return result;
#endif
...
public:
    ...
    constexpr auto operator()(const TEvent& event, TSM& sm, TDeps& deps, TSubs& subs)
    {
        return !call<TEvent, args_t<T, TEvent>, typename TSM::logger_t>::execute(g, event, sm, deps,
                                                                                 subs);
    }
...
template <
    class T1, class T2,
    __BOOST_SML_REQUIRES(concepts::callable<bool, T1>::value&& concepts::callable<bool, T2>::value)>
constexpr auto operator&&(const T1& t1, const T2& t2)
{
    return front::and_<aux::zero_wrapper<T1>, aux::zero_wrapper<T2>>(aux::zero_wrapper<T1>{t1},
                                                                     aux::zero_wrapper<T2>{t2});
}
template <
    class T1, class T2,
    __BOOST_SML_REQUIRES(concepts::callable<bool, T1>::value&& concepts::callable<bool, T2>::value)>
constexpr auto operator||(const T1& t1, const T2& t2)
{
    return front::or_<aux::zero_wrapper<T1>, aux::zero_wrapper<T2>>(aux::zero_wrapper<T1>{t1},
                                                                    aux::zero_wrapper<T2>{t2});
}
template <
    class T1, class T2,
    __BOOST_SML_REQUIRES(concepts::callable<void, T1>::value&& concepts::callable<void, T2>::value)>
constexpr auto operator,(const T1& t1, const T2& t2)
{
    return front::seq_<aux::zero_wrapper<T1>, aux::zero_wrapper<T2>>(aux::zero_wrapper<T1>{t1},
                                                                     aux::zero_wrapper<T2>{t2});
}
namespace front
{
...
auto transitional_impl(T&& t)
    -> aux::always<typename T::dst_state, typename T::src_state, typename T::event,
...
struct state_impl
{
    ...
    constexpr auto operator[](const T& t) const
    {
        return transition_sg<TState, aux::zero_wrapper<T>>{static_cast<const TState&>(*this),
                                                           aux::zero_wrapper<T>{t}};
    }
    template <class T, __BOOST_SML_REQUIRES(concepts::callable<void, T>::value)>
    constexpr auto operator/(const T& t) const
    {
        return transition_sa<TState, aux::zero_wrapper<T>>{static_cast<const TState&>(*this),
                                                           aux::zero_wrapper<T>{t}};
    }
};
template <class TState>
...
struct ignore<E, aux::type_list<Ts...>>
{
    ...
    {
        using type = aux::conditional_t<
            aux::is_same<back::get_event_t<E>,
    ...
    : transition<state<internal>, state<S2>, front::event<back::anonymous>, G, A>
{
    ...
    constexpr transition(const G& g, const A& a)
        : transition<state<internal>, state<S2>, front::event<back::anonymous>, G, A>{g, a}
    {
    }
    template <class T>
...
struct transition<state<S1>, state<S2>>
    : transition<state<S1>, state<S2>, front::event<back::anonymous>, always, none>
{
    constexpr transition(const state<S1>&, const state<S2>&)
        : transition<state<S1>, state<S2>, front::event<back::anonymous>, always, none>{always{},
                                                                                        none{}}
    {
    }
};
...
struct transition_sg<state<S2>, G>
    : transition<state<internal>, state<S2>, front::event<back::anonymous>, G, none>
{
...
struct transition_sa<state<S2>, A>
    : transition<state<internal>, state<S2>, front::event<back::anonymous>, always, A>
{
    using transition<state<internal>, state<S2>, front::event<back::anonymous>, always, A>::a;
    transition_sa(const state<S2>&, const A& a)
        : transition<state<internal>, state<S2>, front::event<back::anonymous>, always, A>{always{},
                                                                                           a}
    {
    }
    template <class T>
...
#else
    (void)aux::swallow{
        0, (sm.current_state_[aux::get_id<state_t, THs>((typename T::initial_states_ids_t*)0)] =
                aux::get_id<state_t, THs>((typename T::states_ids_t*)0),
            0)...};
#endif
}
...
struct transition<state<S1>, state<S2>, front::event<E>, G, A>
{
    ...
    {
        if (call<TEvent, args_t<G, TEvent>, typename SM::logger_t>::execute(g, event, sm, deps,
                                                                            subs))
        {
            sm.process_internal_event(back::on_exit<back::_, TEvent>{event}, deps, subs,
                                      current_state);
            update_current_state(
                sm, deps, subs, current_state,
                aux::get_id<typename SM::state_t, dst_state>((typename SM::states_ids_t*)0),
                state<src_state>{}, state<dst_state>{});
            call<TEvent, args_t<A, TEvent>, typename SM::logger_t>::execute(a, event, sm, deps,
                                                                            subs);
            sm.process_internal_event(back::on_entry<back::_, TEvent>{event}, deps, subs,
                                      current_state);
            return true;
        }
        return false;
...
struct transition<state<internal>, state<S2>, front::event<E>, G, none>
{
    ...
    constexpr bool execute(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs,
                           typename SM::state_t&, Ts&&...)
    {
        return call<TEvent, args_t<G, TEvent>, typename SM::logger_t>::execute(g, event, sm, deps,
                                                                               subs);
    }
    G g;
};
...
template <
    class... Ts,
    __BOOST_SML_REQUIRES(aux::is_same<aux::bool_list<aux::always<Ts>::value...>,
